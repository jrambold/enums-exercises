...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    # initial value is 1
    result = numbers.reduce(1) do |multiple, num|
      multiple * num
    end
  ...
  def test_divide_560_by_a_bunch_of_numbers
    ...
    result = numbers.reduce(560) do |divided, num|
      divided / num
    end
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) do |count, nums|
      count - nums.min
    end
  ...
  def test_add_all_the_second_values_together
    ...
    # initial value is 0
    result = elements.reduce(0) do |sum, value|
      sum + value[1]
    end
